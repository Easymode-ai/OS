...
u32int placement_address = 0;//(u32int)&end;
...
static s32int find_smallest_hole(u32int size, u8int page_align, heap_t *heap)
{
	...
	while (iterator < heap->index.size) {
		...
		// If the user has requested the memory page-aligned
		if (page_align > 0) {
...
void free(void *p, heap_t *heap)
{
	...
	// Get the header and footer assosiated with this pointer.
	...
	if (test_footer->magic == HEAP_MAGIC && test_footer->header->is_hole == 1) {
		...
		footer->header		= header;		// Rewrite our footer to poin to  the new header.
	...
	if (test_header->magic == HEAP_MAGIC && test_header->is_hole) {
		...
		test_footer	= (footer_t*) ( (u32int)test_header + test_header->size - sizeof(footer_t) );	// Rewrite it's footer to point to our header.
		footer		= test_footer;
	...
	if ( (u32int)footer + sizeof(footer_t) == heap->end_address) {
		...
		if (header->size - (old_length - new_length) > 0) {
			...
			while ( (iterator < heap->index.size) && (lookup_ordered_array(iterator, &heap->index) != (void*)test_header) )
				iterator++;
			// If we didn't find ourselvs, we have nothing to remove.
			if (iterator < heap->index.size)
				remove_ordered_array(iterator, &heap->index);
		}
	}
